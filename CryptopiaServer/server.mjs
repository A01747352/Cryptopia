...
async function registration(user, password, age, gender, country, occupation){
    ...
    try {
        ...
        await connection.query('INSERT INTO usuario(username, contrasena, edad, genero, pais, ocupacion) VALUES(?, ?, ?, ?, ?, ?);',
            [user, password, ageValue, genderChar, country, occupation]);
...
app.get('/cryptography/newGame/:userId', async (req, res) => {
    ...
    try {
        ...
        console.error("Error registrando juego de criptografía:", err);
...
app.get('/trivia/newGame/:userId', async (req, res) => {
    ...
    try {
        ...
        const response = rows[0][0];
        console.log("Respuesta de registro de trivia:", response);
        res.send(response);
    } catch (err) {
        console.error("Error registrando juego de trivia:", err);
        const {name, message} = err;
        res.status(500).send({error: name, message});
    } finally {
        if (connection) {
...
app.get('/trivia/getAllQuestions', async (req, res) => {
    ...
    try {
        ...
        console.log(`Se encontraron ${questions.length} preguntas de trivia`);
        ...
        for (const question of questions) {
            ...
            console.log(`Pregunta ID ${question.idPregunta} tiene ${answers.length} respuestas`);
...
app.use((req, res) => {
    ...
    res.status(404).send(`Ruta no encontrada: ${url}`);
});
